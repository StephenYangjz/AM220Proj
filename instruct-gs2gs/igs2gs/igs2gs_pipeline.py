...
from igs2gs.conditioning.CompositDiffusion import CompositDiffusion

from PIL import Image

# Function to save image tensor to a file
def save_image(image_tensor, file_path):
    # Convert tensor to numpy array, then to PIL image
    pil_image = Image.fromarray((image_tensor.squeeze().permute(1, 2, 0).cpu().numpy() * 255).astype('uint8'))
...
class InstructGS2GSPipeline(VanillaPipeline):
    ...
    def __init__(
        ...
        self.composite_diffusion = CompositDiffusion()
    ...
    def get_train_loss_dict(self, step: int):
        ...
        if (not self.makeSquentialEdits):
            ...
            save_image(rendered_image, "current_render.png")
            # conditioning_data = self.datamanager.train_dataset.get_conditioning_data(1)
            depth_data = str(self.datamanager.train_dataset.metadata["depths"][idx])
            composite_masks = [str(path) for path in self.datamanager.train_dataset.metadata["composite_masks"][idx]]
            conditioned_img = self.composite_diffusion.condition("current_render.png", "test.png", depth_data,  *composite_masks)
            to_tensor = transforms.ToTensor()  # Converts to [C, H, W] (Channels, Height, Width)
            conditioned_img = to_tensor(conditioned_img).unsqueeze(0)
            ...
            save_image(conditioned_img, "conditioned.png")
            edited_image = conditioned_img
