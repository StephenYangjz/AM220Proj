...
class InputDataset(Dataset):
    ...
    def get_any_numpy_image(self, image_filename) -> npt.NDArray[np.uint8]:
        """Returns the image of shape (H, W, 3 or 4).
    ...
    def get_any_image_float32(self, path) -> Float[Tensor, "image_height image_width num_channels"]:
        """Returns a 3 channel image in float32 torch.Tensor.

        Args:
            image_idx: The image index in the dataset.
        """
        image = torch.from_numpy(self.get_any_numpy_image(path).astype("float32") / 255.0)
        if self._dataparser_outputs.alpha_color is not None and image.shape[-1] == 4:
            assert (self._dataparser_outputs.alpha_color >= 0).all() and (
                self._dataparser_outputs.alpha_color <= 1
            ).all(), "alpha color given is out of range between [0, 1]."
    ...
    def get_conditioning_data(self, image_idx: int) -> Dict:
        """Method that can be used to process any additional metadata that may be part of the model inputs.

        Args:
            image_idx: The image index in the dataset.
        """
        masks = [self.get_any_image_float32(filename) for filename in self.metadata["composite_masks"][image_idx]]
        depth = self.get_any_image_float32(self.metadata["depths"][image_idx])
        return {"image_idx": image_idx, "depth": depth, "masks":masks}
